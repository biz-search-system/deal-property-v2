...
**将来的な拡張予定**:
顧客のニーズに応じて、組織をさらに細分化できる機能を検討中です。例えば：

- 営業組織 → 一般社員、管理職
- 事務組織 → 一般事務、管理職

各細分化された組織ごとに、閲覧範囲と編集可能範囲を個別に設定できるようにする予定です。MVPでは、まず経営・営業・事務の3組織で運用を開始します。
...
**MVPでの対応**: 事務員が案件情報の入力を主に担当する実態に合わせ、MVPでは事務組織も全項目の編集を可能とします。将来的に、組織をさらに細分化（一般事務・管理職等）した際に、編集権限の詳細な制御を実装する予定です。
...
| 2 | 契約CB待ち | BC業者確定、重説・BC契約書のチェックバック待ち |
| 3 | BC契約待ち | チェックバック完了、BC契約待ち |
| 4 | 決済日待ち | BC契約完了、決済日未確定 |
| 5 | 精算CB待ち | 決済日確定、精算書作成中 |
| 6 | 決済待ち | 精算書完了、決済待ち |
| 7 | 決済完了 | 決済完了 |
...
| # | 項目 | 説明 | 表示仕様 |
|---|------|------|---------|
| 1 | 担当 | 担当者名（複数の場合、バッジで表示） | バッジ表示 |
| 2 | 物件名 | 物件の名称 | 全文表示・固定列 |
| 3 | 号室 | 部屋番号 | 全文表示・固定列 |
| 4 | オーナー | オーナー名 | 全文表示 |
| 5 | A金額 | 入口金額（万円単位） | 右寄せ |
| 6 | 出口 | 出口金額（万円単位） | 右寄せ |
| 7 | 仲手等 | 仲介手数料等（万円単位） | 右寄せ |
| 8 | 利益 | 自動計算された利益額（万円単位） | 右寄せ・強調 |
| 9 | 契約形態 | 契約の形態 | 最大5文字 |
| 10 | B会社 | B会社名 | 最大5文字 |
| 11 | 仲介 | 仲介会社名 | 最大5文字 |
| 12 | 進捗 | 業務ステータス | 最大6文字・クリック編集 |
| 13 | 書類 | 書類ステータス | 全文表示・クリック編集 |
| 14 | 備考 | メモ・注意事項 | クリック編集可 |
| 15 | 操作 | 詳細・編集メニュー | 三点メニュー・固定列 |
...
| 7 | 仲手等 | 仲介手数料等（万円単位） | 右寄せ |
| 8 | 利益 | 自動計算された利益額（万円単位） | 右寄せ・強調 |
| 9 | BC手付 | BC手付金額（万円単位） | 右寄せ |
| 10 | 決済日 | 決済予定日（1/15形式、月のみは「11月予定」） | 中央寄せ |
| 11 | 買取 | 買取会社名 | 最大5文字 |
| 12 | 契約形態 | 契約の形態 | 最大5文字 |
| 13 | B会社 | B会社名 | 最大5文字 |
| 14 | 仲介 | 仲介会社名 | 最大5文字 |
...
- 書類進捗
  - 書類ステータス（書類依頼待ち / 書類取得中 / 全書類取得完了）
  - ※次バージョンで詳細な書類ごとのステータス表示予定
