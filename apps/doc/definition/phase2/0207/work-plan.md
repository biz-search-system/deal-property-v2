...
- `apps/web/components/exit/maisoku/maisoku-editor.tsx` - 画像の位置・サイズ状態管理、テンプレートごとの初期位置定義、画像設定時にアスペクト比を保った自動サイズ調整
- `apps/web/components/exit/maisoku/maisoku-image-uploader.tsx` - 画像アップローダー（クリック/ドラッグ＆ドロップ、差替え、削除）
- `apps/web/components/exit/maisoku/maisoku-preview.tsx` - `react-rnd` によるドラッグ移動・リサイズ対応、画像枠に黒色ボーダー、ホバー時プライマリカラー

**機能詳細**:

- 画像アップロード: クリックまたはドラッグ＆ドロップで設定
- 画像差替え・削除: ホバーで「差替え」「削除」ボタン表示
- 自動サイズ調整: 画像設定時に元画像のアスペクト比を保ちスロット枠内に収める
- ドラッグ移動: プレビュー上で画像を自由に移動（A4枠内制限）
- リサイズ: 画像の四隅・辺をドラッグしてサイズ変更
